...
const val EXTRA_MAP_TITLE = "Extra map Title"
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fabCreateMap.setOnClickListener {
            Log.i(TAG, "tapped FAB")
            showAlertDialog()
        }

        // when user taps item nav to new activity
    }
    ...
    private fun deserializeUserMaps(context: Context) : List<UserMap> {
        Log.i(TAG, "deserialize usermaps")
        val dataFile = getDataFile(context)
        if (!dataFile.exists()) {
            Log.i(TAG, "no data file exists")
            return emptyList()
        }
